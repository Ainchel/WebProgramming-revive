...
조인을 사용하면 2개의 테이블에서 데이터를 조회할 수 있다.

조인은 한 테이블 A의 한 열을 다른 테이블 B에 포함하여 조회할 수 있게 만들어 준다. 이는 데이터의 중복을 막는 것이 목적이다. (필요한 데이터를 한 테이블에 때려박아 데이터 중복이 일어나는 것을 막기 위해서)

내부 조인이라는 조인 형태가 가장 많이 사용된다고 한다. 내부 조인은 두 테이블에 같이 들어가 있는 열을 기준으로 두 테이블을 정보를 함께 조회할 수 있다.

아래는 예시이다.

```
고객 테이블에는 고객 번호, 고객 이름이 저장되어 있다.
주문 테이블에는 고객 번호, 주문 번호, 주문 날짜가 저장되어 있다.
이럴 경우 같이 들어가 있는 열인 '고객 번호' 를 통해 두 테이블의 정보인 고객 번호, 고객 이름, 주문 번호, 주문 날짜 등을 조회할 수 있다.
```

내부 조인 쿼리문의 예시는 다음과 같다.

```
SELECT 열 << 내가 찾고 싶은 하는 데이터 열을 지정한다.
FROM 테이블1 << 테이블 1 안에서 찾는다.
  INNER JOIN 테이블2 ON 테이블1.열 = 테이블2.열 << 테이블1과 같은 열을 가지고 있는 테이블2와 연결해서, 테이블2에서도 같은 값을 갖고 있는 데이터를 가져온다.
WHERE 검색조건 << 가져온 데이터에서 검색조건에 맞게 필터링한다.
```

다시 한 번 흐름을 정리하면 다음과 같다.

```
1. FROM에 들어간 테이블 A와 INNER JOIN에 들어간 테이블 B를 합쳐, ON으로 적은 조건에 맞는 행만 정리된 가상 테이블을 만든다
2. 가상 테이블에서 WHERE로 제시한 조건에 맞는 행만 다시 골라낸다
3. 골라낸 테이블 데이터 내에서 SELECT로 제시한 열만 정리해서 보여준다
```

** ON은 조인시키는 테이블의 조건을 적기 위해 사용한다. WHERE는 조인시킨 이후 데이터에서 조건을 걸어 필터링하는 데 사용한다. 둘의 차이는 명확하니 기억해두자.

** 두 테이블을 조인시킨 상태에서 SELECT문을 사용할 경우, 두 테이블에 모두 있는 열은 이름을 구분해서(테이블이름.열이름 같은 방식으로) 적어주어야 에러가 발생하지 않는다.

조인 조건으로 열을 2개 이상 사용할 수도 있다. AND나 OR 등의 연산자도 사용할 수 있다고 한다. 또한 열 이름이 같지 않더라도 데이터 유형이 같다면 조건으로 걸 수 있다.

내부 조인으로 테이블을 3개 이상 조인시킬 수도 있다. 어쨌든 전제는 테이블 3개에 같은 데이터 유형을 가진 열이 있어야 한다는 것이다. 예시는 아래와 같다.
...
내부조인은 같은 값 조건을 가지는 데이터를 조회해야 하기 떄문에 NULL이 발생하지 않는다.

외부 조인은 두 테이블을 조인하되, 둘 중 한 테이블의 데이터는 조건 없이 전부 가져온다. 예시는 다음과 같다.

```
SELECT 열
FROM 테이블1 LEFT/RIGHT/FULL(셋 중 하나만 입력)
OUTER JOIN 테이블2 ON 테이블1.열 = 테이블2.열
WHERE 검색조건
```

여기서 LEFT는 FROM에 들어간 테이블의 전체 데이터를, RIGHT는 JOIN에 들어간 테이블의 전체 데이터를 가져온다. FULL은 MySQL에선 지원 안 하나??

조금 복잡하지만, 내부 조인의 결과에서 OUTER로 지정한 방향의 데이터만 추가로 NULL 포함해서 채운다는 감각으로 이해하자. 나중에 더 찾아보기

자주 사용하지는 않지만 각 테이블의 모든 경우의 수를 조합한 교차 조인도 있다. 나중에 찾아보자.

한 테이블과 자기 자신을 조인시키는 셀프 조인도 있다. 하나의 테이블을 쓰기 때문에 반드시 별칭을 사용해 자신 테이블을 구분지어줘야 한다. 자신 테이블 내 특정 값을 비교한 의미를 낼 때 필요하다고 하는데 너무 어렵다...

같은 테이블 내의 데이터를 연결하거나 서로 비교해야 할 경우가 있으며, 이건 일반적인 SELECT로는 해결할 수 없다. 이 때 셀프 조인을 사용한다는 것만 기억해두자.

# 쿼리 안에 또 다른 쿼리, 서브 쿼리(200 페이지 ~ 211 페이지지)

서브 쿼리란 쿼리 안에 포함되어 있는 또 다른 쿼리를 말한다.

서브 쿼리는 조인하지 않은 상태의 다른 테이블의 조건에 맞는 행을 찾거나 조인 결과를 다시 조인하는 등에 사용한다. 서브쿼리의 특징은 아래와 같다.

```
1. 반드시 소괄호로 감싸 사용한다.
2. 주 쿼리를 실행하기 전에 한 번만 실행된다.
3. 비교 연산자와 함께 사용할 경우 연산자 오른쪽에 서브쿼리를 기술해야 한다.
4. ORDER BY 절을 통해 정렬을 할 수 없다.
```

WHERE 절에 사용하는 서브 쿼리는 중첩 서브 쿼리라고 한다. 이 경우 중첩 서브 쿼리는 또 다른 SELECT문의 결과를 주 쿼리의 조건값으로 사용하게 된다.

서브 쿼리의 결과로 2행 이상 반환되는 경우를 다중 행 서브쿼리라고 한다. 이를 위해선 IN, NOT IN, ANY, EXISTS, ALL 등의 다중 행 연사자를 활용해야 한다. 지금 공부하기엔 양이 많고 복잡하므로 다음에 필요할 떄 찾아보자.

FROM 절에 사용한 서브쿼리의 결과는 테이블처럼 사용되어 다른 테이블에 조인할 수 있다. 즉 INNER JOIN에 사용할(조인시킬) 테이블로 서브쿼리의 결과물 테이블을 사용할 수 있다는 것이다.

SELECT 절에서 사용하는 서브쿼리는 스칼라 서브 쿼리라고 한다.

# 공통 테이블 표현식이란? (212 페이지 ~ 214 페이지) 진행 중

공통 테이블 표현식은 실제 데이터베이스에 테이블을 생성하는 것은 아니지만, 쿼리 실행 결과를 테이블처럼 활용하기 위한 논리적인 테이블을 만들 때 활용한다. CTE라고도 한다.

보통 데이터베이스에 없는 테이블을 임시로 만들 때 사용하며, 그 다음 SELECT문에 해당 임시 테이블을 사용해 데이터를 조회할 수 있다.

결과를 재사용하는 재귀 공통 테이블 표현식이라는 것도 있다고 한다. 나중에 찾아보기.

기본 형식은 다음과 같다.

```
WITH 테이블 (열1, 열2, ......) << 완성시킬 테이블의 정보를 담는다
AS
(
  SELECT문 << CTE 테이블에 들어갈 데이터를 조회
)
SELECT 열 FROM 테이블;
```

UNION과 UNION ALL 연산자는 여러 CTE를 결합한다. 여러 쿼리의 결과를 하나의 데이터 집합으로 결합시킬 수 있다.

일반 CTE에서 INTERSECT 연산자를 사용하면 각 쿼리에서 변환한 결과 중 중복값을 걸러 반환시켜준다.

=====================JPA 진행을 위해 이 책 정리는 보류한다. 하지만 서브쿼리 등 SQL와 쿼리에 대한 이해는 백엔드에서 필수 기술이므로 나중에 더 공부 해야 한다......
